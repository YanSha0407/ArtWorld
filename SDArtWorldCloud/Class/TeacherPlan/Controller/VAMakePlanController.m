...
-(UIButton *)nextBtn{
    if (!_nextBtn) {
        ...
        [_nextBtn setTitleColor:VAMainTitleColor forState:UIControlStateNormal];
        [_nextBtn addTarget:self action:@selector(clickNextBtn) forControlEvents:UIControlEventTouchUpInside];
        _nextBtn.titleLabel.font = UIFontMake(18444);
    ...
    if (self.courseNameTextFiled.text.length) {
        if (kIsNetwork) {
           NSDictionary *dic = @{@"class_period_id":[NSNumber numberWithInteger:self.courseModel.class_period_id.class_period_id],@"class_id":[NSNumber numberWithInteger:self.classModel.class_id],@"category_id":[NSNumber numberWithInteger:self.subjectModel.category_id],@"admin_id":[VAAccountManager getAccount].admin_id,@"course_material_name":self.courseNameTextFiled.text,@"class_content":self.courseNameTextFiled.text,@"time":self.planDate,@"instruction_method_id":self.formModel.method_id};
            ...
            [PPNetworkHelper GET:kMakePlanData parameters:dic success:^(id responseObject) {
...
-(void)getClassFromData{
    if (kIsNetwork) {
        [PPNetworkHelper GET:kGetClassFormData parameters:nil success:^(id responseObject) {
            ...
            if (result) {
                [self.classFormMutableArray addObjectsFromArray:[VASubjectModel mj_objectArrayWithKeyValuesArray:result]];
...
-(void)viewWillLayoutSubviews{
    ...
    self.formName.frame = CGRectMake(self.courseName.left, self.subjectName.bottom + 90, self.courseName.width, self.courseName.height);
    self.formView.frame = CGRectMake(self.courseName.right, self.subjectName.bottom + 70 , KSystemWidth - self.courseName.right - 20, 100);
    self.nextBtn.frame = CGRectMake(self.courseName.left, self.formView.bottom + 100 , KSystemWidth - 40, 44);
}
@end
