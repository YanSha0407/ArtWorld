...
@interface VAVideoGroupController ()
@property(nonatomic,strong)NSString *groupName;
@end

@implementation VAVideoGroupController
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if (self.isShowAddBtu && indexPath.row == 0) {
        ...
        VABaseVideoListController *basePicture = [[VABaseVideoListController alloc]init];
        basePicture.title = @"视频分组";
...
-(void)addPictureGroup:(NSString *)name{
    if (kIsNetwork) {
        NSDictionary *dic = @{@"name":name,@"course_material_id":self.planModel.course_material_id,@"type":@"20"};
